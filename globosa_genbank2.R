...
setwd("~/MyDocs/MEGA/UCI/Schiedea/Analysis/scent/rmbl/Schiedea/genbank")
...
gb <-  gb_seq <- list()
gbr <- lapply(genes, function(g) gbRecord(paste0(g,"/sequence.gb"))) #slow
names(gbr) <- genes
for (g in genes) {
...
gb.meta <- gb %>% 
  lapply(tibble::rownames_to_column,var="accession") %>% 
  bind_rows( .id = "gene") %>% 
  mutate(pop.hap = factor(paste(organism, pop_variant, haplotype))) %>%
  dplyr::select(-c(mol_type, organelle)) %>%
  spread(gene, accession, convert=T)
levels(gb.meta$pop_variant)[17]<- "102HU"# 102HU == Huelo #Could also lump #Kanehoalani: 964 is Kawelo is Wood 13885
gb.meta$individual <- factor(paste(gb.meta$pop_variant, ifelse(
  substr(gb.meta$haplotype, nchar(gb.meta$haplotype),nchar(gb.meta$haplotype)) %in% c("A", "B"), 
  substr(gb.meta$haplotype, 1, nchar(gb.meta$haplotype)-1), 
  gb.meta$haplotype)))
...
gb.meta.cp <- gb.meta %>% 
  dplyr::filter(!is.na(`psbM-trnD`) & organism=="Schiedea globosa") %>%
  arrange(`psbM-trnD`)
write.csv(gb.meta.cp, "Wallace2009_Sglobosa_individuals.csv")
...
library(ape)
library(vegan)
aln.paths <- setNames(c("./ncpGS/FJ496426.1 and 115 other sequences.aln",
...
save(meand,genpofaddist,pofaddist.avg,avgd,aln.meta.unique,gb.meta,genes, file="mean_gen_dist.Rdata")
...
#Plot phylo
cairo_pdf("nj_pofad.pdf", width=10, height=10)
par(mar=c(0,0,0,0))
...
cairo_pdf("nj_pofad_concat.pdf", width=10, height=10)#concatenated tree looks very different! 
par(mar=c(0,0,0,0))
...
cairo_pdf("nj_pofad_boot.pdf", width=10, height=10)#concatenated tree looks very different! 
par(mar=c(0,0,0,0))
plot(phy.pofad, tip.color=gen.pops.pal[as.integer(aln.meta.unique[[1]]$pop_variant)])
...
#NeighborNet
library(phangorn)
nn.pofad <- neighborNet(pofaddist.avg)
cairo_pdf("nn_pofad.pdf", width=20, height=20)
set.seed(11)
...
phyg.pofad <- keep.tip(phy.pofad, as.character(aln.meta.glob$individual))
...
library(sf)
pops <- st_read("http://www.google.com/maps/d/u/0/kml?forcekml=1&mid=1nrFQN_CJTffw2Smd3-dD9C-sbqU&lid=2WoGNCc9HTw")#"Schiedea populations.kml" #From Google My Map
...
ages <- read.delim("island_Ages.csv")
...
library(mapdata)
phyg.map <- phylo.to.map(phyg.pofad, phyg.coord, rotate=T, 
...
cairo_pdf("map_phylo_pofad_avg_nj_spcol_bracket.pdf", width=9.5, height=12)
#png("map_phylo_pofad_avg_nj_spcol_bracket.png", width=9.5, height=12, units="in", res=300)
plot.phylo.to.map2(phyg.map, 
                   ...
                   pch=NA)
#points(mg[-(26:28),])
with(all.glob.pops, points(X,Y, pch=c(21,22,23)[Topography]))
...
legend(x=-158.5, y=20, title="Population",
       ...
       pch=c(21,22,23)[glob.pops$Topography], pt.cex=2, bty="n", title.adj=0)
legend(x=-158.5, y=20.5, title="Topography", 
       legend=levels(glob.pops$Topography), 
...
remove(gbr)
#save.image("globosa_genbank.Rdata")

######SCRAPS#####
library(genbankr)
gb <- readGenBank(file="sequence.gb")#doesn't work


library(xml2)
gb <- read_xml("sequence.xml")
xml_text( xml_find_all( gb, "//ADDRESS" ) )

library(XML)
gb <- xmlParse("sequence.xml")
gbl <- xmlToList(gb)

library(rentrez)
glob.search <- entrez_search(db = 'PopSet', term = '229904981', retmax = 1000)
ps <- entrez_fetch(db = 'PopSet', id = 229904981, 
                   rettype = 'native', retmode = 'xml', 
                   parsed = TRUE)
xl <- xmlToList(ps)
x <- unlist(xl)
# all nodes share this in common:
basePath <- 'Bioseq-set_seq-set.Seq-entry.Seq-entry_seq.Bioseq-set.Bioseq-set_seq-set'
